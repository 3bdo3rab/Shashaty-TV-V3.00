...
export const ScheduleNotifier: React.FC<ScheduleNotifierProps> = ({
  ...
  const playAlertSoundAndSpeech = (title: string) => {
    try {
      ...
      // 2. Speak Arabic Notification via SpeechSynthesis API
      if ('speechSynthesis' in window) {
        window.speechSynthesis.cancel(); // clear previous queue
        const utterance = new SpeechSynthesisUtterance(`اقترب موعد عرض: ${title}`);
  ...
  // Option 1: WATCH NOW (مشاهدة القناة الحية)
  const handlePlayNow = () => {
    ...
    if (targetChannel) {
      ...
      if (nowPlaying) {
        ...
        if (linkedWl) {
          ...
          if (files.length > 0) {
            onPlay(files[0], files[0]?.name || files[0]?.title || 'عرض حي', targetChannel.title, files, 0, undefined, linkedWl.id, 0, targetChannel.id);
          }
        }
      }
    }
  ...
  const handleSnoozeMinutes = (minutesToDelay: number) => {
    ...
    try {
      if ('speechSynthesis' in window) {
        ...
        const utterance = new SpeechSynthesisUtterance(`تم تأجيل موعد ${entry.title} إلى الساعة ${newTimeStr}`);
        utterance.lang = 'ar-SA';
        window.speechSynthesis.speak(utterance);
      }
    } catch {}
  ...
  const handleCancelSlot = () => {
    ...
    try {
      if ('speechSynthesis' in window) {
        window.speechSynthesis.cancel();
        const utterance = new SpeechSynthesisUtterance(`تم إلغاء تنبيه الموعد المجدول لـ ${entry.title}`);
        utterance.lang = 'ar-SA';
        window.speechSynthesis.speak(utterance);
      }
    } catch {}
