...
export const getTauriWindow = async () => {
  ...
  try {
    if ((window as any).__TAURI__?.window) {
      ...
      if (typeof w.getCurrentWindow === 'function') {
    ...
    const tauriWindowModule: any = await import('@tauri-apps/api/window');
    if (typeof tauriWindowModule.getCurrentWindow === 'function') {
      return tauriWindowModule.getCurrentWindow();
    }
    if (tauriWindowModule.appWindow) {
      return tauriWindowModule.appWindow;
    }
  } catch (e) {
