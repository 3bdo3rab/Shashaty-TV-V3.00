...
export const ChannelsView: React.FC<ChannelsViewProps> = ({
    ...
    if (smartStrategy === 'category') {
      ...
      targetDays.forEach((dayOfWeek, dayIndex) => {
        ...
        sortedCategorySlots.forEach((slotConfig) => {
          ...
          const matchedChan = selectedWl 
            ...
            : resolvedChannels.find(c => c.modes?.includes(slotConfig.mode)) || resolvedChannels[0];

          const meta = CATEGORY_NAMES_WITH_ICONS[slotConfig.mode];
          const chanTitle = matchedChan ? matchedChan.title : (meta?.label || `قناة ${slotConfig.mode}`);
      ...
      targetDays.forEach((dayOfWeek, dayIndex) => {
        ...
        selectedTimes.forEach((timeStr, slotIndex) => {
          ...
          const matchedChan = findMatchingChannel(selectedWl) || resolvedChannels[0];
  ...
  const handleSaveScheduleSlot = async () => {
    ...
    const finalTitle = slotTitle.trim() || selectedChan?.title || 'موعد قناة مباشر';
    ...
    const updatedEntry: WeeklyScheduleEntry = {
      ...
      mode: (slotMode as Mode) || selectedChan?.modes?.[0] || undefined,
    };
  ...
  // Direct Play Channel Live from Schedule Entry
      ...
      {initialTab === 'channels' ? (
        ...
        <>
          ...
          <section className="relative z-10 space-y-8">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-zinc-950/70 p-6 rounded-3xl border border-white/10 backdrop-blur-xl shadow-2xl">
              <div>
                <div className="flex items-center gap-3 mb-2 flex-wrap">
              ...
              <div className="sticky top-2 z-30 flex flex-wrap items-center gap-2.5 sm:gap-3 p-2 bg-zinc-950/90 rounded-2xl border border-white/20 backdrop-blur-xl shadow-2xl">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white/10 hover:bg-white/20 text-white font-extrabold transition-all text-xs sm:text-sm cursor-pointer border border-white/20 shrink-0 hover:scale-105 active:scale-95"
                ...
                {schedules.length > 0 && (
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-3 rounded-xl bg-red-500/20 hover:bg-red-500 hover:text-white text-red-300 font-extrabold transition-all text-xs sm:text-sm cursor-pointer border border-red-500/30 shrink-0 hover:scale-105 active:scale-95 shadow-sm"
                            ...
                            <div className="flex items-center gap-2.5 flex-wrap text-xs text-white/60">
                              ...
                              <div className="flex items-center gap-1.5 bg-amber-400/10 px-3 py-1 rounded-xl border border-amber-400/30">
                                ...
                                <select
                                  ...
                                  className="bg-transparent text-amber-200 font-extrabold focus:outline-none cursor-pointer text-xs"
                                >
                              ...
                              <span className="px-2.5 py-1 rounded-xl bg-white/10 text-white/80 border border-white/10 text-xs font-bold flex items-center gap-1">
                                <span>⏱️</span>
                                <span>{slot.durationMinutes || slot.transitionMinutes || 60} دقيقة بث حي</span>
                              </span>
                            </div>
                          </div>
                        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto min-h-0 space-y-4 pr-1 pl-1.5 my-1 text-right">
                ...
                {/* 2. PRIMARY FIELD: CHANNEL SELECTOR (اختيار القناة) */}
                <div className="p-4 rounded-2xl bg-amber-400/10 border border-amber-400/30 space-y-2">
                  <label className="block text-xs font-black text-amber-300 flex items-center gap-1.5">
                    ...
                    <span>اختر القناة المخصصة للبث (العنصر الرئيسي)</span>
                  </label>
                  <select
                    value={slotChannelId}
                    onChange={(e) => {
