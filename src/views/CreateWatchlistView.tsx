                              ...
                              >
                                ...
                                {isSExpanded && (
                                  <div className="pr-3 border-r border-white/10 space-y-0.5 mr-2 py-1">
                                    {s.files.map((f, fIdx) => (
                                      <div key={fIdx} className="flex items-center gap-2 text-[11px] text-white/70 py-1 px-2 hover:bg-white/5 rounded-lg transition-colors">
                                        <FileVideo className="w-3 h-3 text-emerald-400 shrink-0" />
...
export const CreateWatchlistView: React.FC<CreateWatchlistViewProps> = ({ 
    ...
    if ('showDirectoryPicker' in window) {
      try {
        ...
        if (dirHandle) {
          ...
          const readFilesFromHandle = async (handle: any, currentPath = ''): Promise<File[]> => {
            let filesList: File[] = [];
            for await (const entry of handle.values()) {
              if (entry.kind === 'file') {
                ...
                const subFiles = await readFilesFromHandle(entry, subPath);
          ...
          const allFiles = await readFilesFromHandle(dirHandle);
  ...
  return (
    <motion.div 
      ...
      exit={{ opacity: 0 }}
      className="p-4 sm:p-8 lg:p-12 h-full flex flex-col relative w-full pb-32 md:pb-12"
    >
      <ProcessingRing 
        isVisible={isProcessing} 
        message="جاري استخراج المصغرة وحفظ قائمة التشغيل..." 
        subMessage="يتم قراءة خصائص ملفات الفيديوهات والمجلد" 
      />
      ...
      <div className="flex-1 flex flex-col xl:flex-row gap-8">
              ...
              {MODE_OPTIONS.map((m) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-2xl border text-right flex flex-col items-center justify-center gap-2 transition-all cursor-pointer ${
                      isSel
        ...
        {/* Drag & Drop Area */}
        <div 
          onClick={!importedFolder ? handleAddFolderClick : undefined}
          onDragOver={(e) => e.preventDefault()}
          onDrop={importedFolder ? (e) => e.preventDefault() : handleDrop}
          className={`flex-1 glass-card rounded-[2rem] border-2 border-dashed ${importedFolder ? 'border-green-500/50 bg-green-500/5 p-4 sm:p-8' : 'border-white/20 hover:border-white/50 hover:bg-white/5 cursor-pointer p-12'} flex flex-col items-center justify-center text-center group transition-all relative overflow-hidden`}
        >
          ...
          {importedFolder ? (
            ...
            <>
              ...
              <h3 className="text-3xl font-extrabold mb-8 tracking-wide">اختر المجلد الفرعي (قائمة التشغيل)</h3>
              
              <button 
                onClick={(e) => { e.stopPropagation(); handleAddFolderClick(); }}
                className={`glass px-12 py-5 rounded-2xl flex items-center gap-4 transition-all font-black text-xl sm:text-2xl relative z-10 shadow-2xl hover:scale-105 active:scale-95 cursor-pointer ${
      ...
      {/* Info Notice Box Moved Below Card */}
      {!importedFolder && (
        <div className="mt-6 glass-card border border-white/10 rounded-2xl p-5 text-center max-w-2xl mx-auto shadow-xl backdrop-blur-xl">
          <p className="text-base text-white/90 font-bold mb-1.5 flex items-center justify-center gap-2">
            <span className="bg-indigo-500/20 text-indigo-300 p-1.5 rounded-lg shrink-0">
              <FolderOpen className="w-5 h-5" />
            </span>
            <span>المجلد الفرعي هو مجلد خاص بمسلسل أو برنامج واحد (مثل مجلد "Breaking Bad").</span>
          </p>
          <p className="text-xs sm:text-sm text-white/70 leading-relaxed">
      ...
      <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
        {isProcessing && (
          <div className="w-full sm:max-w-md bg-white/10 border border-amber-400/40 rounded-2xl p-3.5 flex items-center gap-3">
        ...
        <button 
          ...
          className={`w-full sm:w-auto px-12 py-4 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(255,255,255,0.3)] ${
