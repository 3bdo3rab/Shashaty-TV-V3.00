...
export const PlayerView: React.FC<PlayerViewProps> = ({ 
  ...
  // Video Picture & Color Settings State
  ...
  const handleLoadedMetadata = () => {
    if (videoRef.current) {
      ...
      // Smart resume from saved initialTime if provided
      if (!initialSeekDone && initialTime && initialTime > 0 && initialTime < videoRef.current.duration) {
  ...
  useEffect(() => {
    ...
    async function loadVideo() {
      ...
      try {
        if (target instanceof File || target instanceof Blob || (target && typeof target === 'object' && target.constructor?.name === 'File')) {
          createdBlobUrl = URL.createObjectURL(target);
          setVideoUrl(createdBlobUrl);
        } else if (target?.kind === 'file' && typeof target.getFile === 'function') {
          const fileData = await target.getFile();
          createdBlobUrl = URL.createObjectURL(fileData);
          setVideoUrl(createdBlobUrl);
  ...
  const triggerNextEpisodeBumper = async () => {
    if (files && currentIndex < files.length - 1) {
  ...
  const handleVideoEnded = () => {
    ...
    if (currItem) {
      ...
      if (!shouldTransition) {
        ...
        if (nextInSameSlot !== -1) {
          playItemAtIndex(nextInSameSlot);
          return;
        } else {
        ...
        if (nextSlotItemIdx !== -1) {
          playItemAtIndex(nextSlotItemIdx);
          return;
        } else {
    ...
    if (files && currentIndex < files.length - 1) {
      if (isTransitionBumperEnabled()) {
        triggerNextEpisodeBumper();
      } else {
        handleNext();
      }
    } else {
  ...
  const speakNextVoice = () => {
    if (typeof window !== 'undefined' && 'speechSynthesis' in window) {
      try {
        window.speechSynthesis.cancel();
        const utterance = new SpeechSynthesisUtterance('التالي');
  ...
  // Trigger speech synthesis voice when bumper opens
  useEffect(() => {
    if (showTransitionBumper) {
      speakNextVoice();
    }
                ...
                {files && files.length > 0 ? (
                  files.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-mono text-sm shrink-0 ${
                          isCurrent ? 'bg-black text-white' : 'bg-white/10'
                        }`}>
                          {isCurrent ? <Check className="w-4 h-4 text-white" /> : idx + 1}
                        </div>
        ...
        {showTransitionBumper && (
          ...
          >
            {(() => {
              ...
              const nextCover = nextEpisodeInfo?.coverImage || nextItem?.coverImage || nextItem?.thumbnail || nextItem?.poster || getEpisodeInspiredCover(nextTitle, nextWatchlist, nextItem ? [nextItem] : []);
              ...
              return (
                <>
                  ...
                  <div className="relative z-10 flex items-center justify-between w-full max-w-7xl mx-auto pt-2">
                    ...
                    <button
                      type="button"
                      onClick={speakNextVoice}
                      className="glass px-5 py-2.5 rounded-full text-white font-extrabold text-sm flex items-center gap-2 hover:bg-white/30 transition-all shadow-xl cursor-pointer"
                      title="نطق الصوت البشري (التالي 🔊)"
                    >
                  ...
                  <div className="relative z-10 flex flex-col items-center text-center my-auto w-full max-w-6xl mx-auto px-4 py-6">
                    ...
                    {/* رقم / اسم الحلقة (كبير جداً جلي الشاشة) */}
                    <h1 className="text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-black text-white drop-shadow-[0_8px_40px_rgba(0,0,0,1)] tracking-tight leading-none my-4">
                      {nextTitle}
                    </h1>
