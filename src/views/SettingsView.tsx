...
export const SettingsView: React.FC<SettingsViewProps> = ({ 
  ...
  const [activeModal, setActiveModal] = useState<'folders' | 'modes' | 'autoplay' | 'backup' | 'tauri' | 'qa' | null>(null);

  const addFolderInputRef = useRef<HTMLInputElement>(null);
  const editFolderInputRef = useRef<HTMLInputElement>(null);
  const backupInputRef = useRef<HTMLInputElement>(null);
  
  const [parentDirNames, setParentDirNames] = useState<string[]>([]);
  const [availableChildFolders, setAvailableChildFolders] = useState<{parentName: string, parentHandle: any, childName: string, childHandle: any}[]>([]);
  const [isExploringParents, setIsExploringParents] = useState(false);
  const [addingFolderId, setAddingFolderId] = useState<string | null>(null);
  const [exploreProgress, setExploreProgress] = useState({ text: '', percentage: 0 });

  useEffect(() => {
    store.getParentDirectoryHandles().then(handles => {
      if (handles && handles.length > 0) {
        setParentDirNames(handles.map((h: any) => h.name).filter(Boolean));
      }
    }).catch(console.error);
  }, []);

  // Managed Folders State
  const [folders, setFolders] = useState<LocalFolder[]>(() => {
    try {
      const saved = localStorage.getItem('app_local_folders');
      return saved ? JSON.parse(saved) : [
        { id: '1', name: 'مجلد المسلسلات الرئيسي', path: '/Media/Series', filesCount: 150, lastScanned: 'منذ ساعتين' },
        { id: '2', name: 'مجلد الأفلام', path: '/Media/Movies', filesCount: 42, lastScanned: 'أمس' },
        { id: '3', name: 'الوثائقيات والمعرفة', path: '/Media/Documentaries', filesCount: 18, lastScanned: 'منذ أسبوع' },
      ];
    } catch {
      return [
        { id: '1', name: 'مجلد المسلسلات الرئيسي', path: '/Media/Series', filesCount: 150, lastScanned: 'منذ ساعتين' },
  ...
  useEffect(() => {
    try {
      localStorage.setItem('app_local_folders', JSON.stringify(folders));
    } catch (err) {
  ...
  const handleToggleAlwaysOnTop = (enabled: boolean) => {
    ...
    try {
      ...
      console.error('Failed to save always on top state:', err);
    }
    setAlwaysOnTop(enabled);
  };

  const handleToggleAutostartWindows = (enabled: boolean) => {
    setAutostartWindowsState(enabled);
    try {
      localStorage.setItem('app_autostart_windows', String(enabled));
    } catch (err) {
      console.error('Failed to save autostart state:', err);
    }
  };
  ...
  const handleRescanFolders = () => {
    setIsScanning(true);
    setTimeout(() => {
      setIsScanning(false);
      setFolders(prev => prev.map(f => ({ ...f, lastScanned: 'تم الفحص للتو' })));
    }, 1200);
  };

  const handlePickNewFolderFromDisk = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (files && files.length > 0) {
      const firstPath = files[0].webkitRelativePath || '';
      const folderName = firstPath.split('/')[0] || 'مجلد جديد';
      setNewFolderName(folderName);
      setNewFolderPath(`/${firstPath.split('/')[0] || folderName}`);
    }
  };

  const handlePickEditFolderFromDisk = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (files && files.length > 0) {
      const firstPath = files[0].webkitRelativePath || '';
      const folderName = firstPath.split('/')[0] || 'مجلد مخصص';
      setEditFolderName(folderName);
      setEditFolderPath(`/${firstPath.split('/')[0] || folderName}`);
    }
  };

  const handleExploreParentDirectories = async () => {
    try {
      setIsExploringParents(true);
      setExploreProgress({ text: 'جارٍ استكشاف المجلدات الأب...', percentage: 10 });
      const existingHandles = await store.getParentDirectoryHandles();
      if (existingHandles.length === 0) {
        await showAlert('يرجى إضافة مجلد أب أولاً.');
        setIsExploringParents(false);
        return;
      }

      const allChildren: {parentName: string, parentHandle: any, childName: string, childHandle: any}[] = [];
      let parentIdx = 0;
      
      for (const parentHandle of existingHandles) {
        parentIdx++;
        setExploreProgress({ text: `فحص ${parentHandle.name}... (${parentIdx}/${existingHandles.length})`, percentage: 10 + (parentIdx / existingHandles.length) * 40 });
        
        try {
          const iterator = typeof parentHandle.values === 'function' ? parentHandle.values() : (parentHandle as any).entries();
          ...
          console.error(`Error exploring parent ${parentHandle.name}`, e);
        }
      }
      
      // Filter out those already added
      setExploreProgress({ text: 'مقارنة مع القوائم الحالية...', percentage: 70 });
      const existingWatchlists = await store.getWatchlists();
      const existingTitles = existingWatchlists.map(w => w.title);
      
      const newAvailable = allChildren.filter(c => !existingTitles.includes(c.childName));
      setAvailableChildFolders(newAvailable);
      setExploreProgress({ text: 'اكتمل الفحص', percentage: 100 });
      ...
      console.error(e);
      await showAlert('حدث خطأ أثناء استكشاف المجلدات.');
      setIsExploringParents(false);
    }
  };

  const handleAddChildFolder = async (child: {parentName: string, parentHandle: any, childName: string, childHandle: any}) => {
    try {
      setAddingFolderId(`${child.parentName}-${child.childName}`);
      
      // Need to scan files in this child
      const { createWatchlistsFromDirectoryHandle } = await import('../utils/fileSystem');
      const newWatchlists = await createWatchlistsFromDirectoryHandle(child.childHandle, (currentMode || 'family') as Mode);
      
      if (newWatchlists.length > 0) {
        const existingWatchlists = await store.getWatchlists();
        await store.setWatchlists([...existingWatchlists, ...newWatchlists]);
        
        // Remove from available
        setAvailableChildFolders(prev => prev.filter(c => c.childName !== child.childName));
        
        await showAlert(`تمت إضافة "${child.childName}" كقائمة تشغيل بنجاح!`);
      } else {
        await showAlert('لم يتم العثور على أي ملفات ميديا مدعومة في هذا المجلد.');
      }
    } catch (e) {
      console.error(e);
      await showAlert('حدث خطأ أثناء الإضافة.');
    } finally {
  ...
  const handleSetParentDirectory = async () => {
    try {
      ...
      if (dirHandle) {
         ...
         if (existingHandles.find(h => h.name === dirHandle.name)) {
            await showAlert('تمت إضافة هذا المجلد مسبقاً.');
            return;
         }
         
         const newHandles = [...existingHandles, dirHandle];
         await store.setParentDirectoryHandles(newHandles);
         setParentDirNames(newHandles.map(h => h.name).filter(Boolean));
         await showAlert('تمت إضافة المجلد الأب بنجاح. يمكنك الآن استكشاف مجلداته الفرعية وإضافتها كقوائم تشغيل.');
      }
    } catch (e: any) {
  ...
  const handleAddFolder = async () => {
    if (!newFolderName.trim() || !newFolderPath.trim()) {
      await showAlert('يرجى إدخال اسم المجلد والمسار.');
      return;
    }
    const newF: LocalFolder = {
      id: Date.now().toString(),
      name: newFolderName,
      path: newFolderPath,
      filesCount: Math.floor(Math.random() * 50) + 10,
      lastScanned: 'تم الإضافة للتو',
    };
  ...
  const handleStartEditFolder = (folder: LocalFolder) => {
  ...
  const handleSaveFolderEdit = async (id: string) => {
    if (!editFolderName.trim() || !editFolderPath.trim()) {
      await showAlert('يرجى ملء اسم المجلد والمسار.');
      return;
    }
  ...
  const sections = [
    ...
    { id: 'qa', title: 'وضع الفحص والضمان (QA Mode)', icon: ShieldCheck, desc: qaInspectionEnabled ? 'مفعّل: جاهز لفحص كامل وظائف المكتبة والقنوات والمشغل' : 'معطل: مفتاح تفعيل للمطور لمحاكاة الاستخدام واكتشاف الأخطاء' },
  ];
  ...
  return (
    ...
    >
      <ProcessingRing 
        ...
        message="جاري فحص المجلدات واستخراج الوسائط..." 
        subMessage="يتم تحليل شجرة المجلدات وتحديث ملفات الميديا" 
      />
        ...
        {activeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/75 backdrop-blur-md">
            ...
            >
              ...
              {/* QA INSPECTION MODAL CONTENT */}
              {activeModal === 'qa' && (
                <div>
                  <div className="flex items-center gap-4 mb-6 pl-14">
                    <div className="p-4 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-2xl shrink-0">
                      <ShieldCheck className="w-8 h-8" />
                    </div>
                    <div>
                      <h2 className="text-3xl font-bold text-white">وضع الفحص والتأكد من الجودة (QA Mode)</h2>
                      <p className="text-white/60 text-sm mt-1">مخصص للمطور لفحص شجرة المحتوى الحقيقية واكتشاف المشاكل تلقائياً</p>
                    </div>
                  </div>

                  <div className="space-y-6">
                    {/* Toggle Switch */}
                    <div className="flex justify-between items-center p-5 glass rounded-2xl border border-emerald-500/30 bg-emerald-500/5">
                      <div>
                        <h4 className="font-bold text-base text-white">تفعيل وضع الفحص (Inspection / QA Mode)</h4>
                        <p className="text-xs text-white/60 mt-1">يُظهر زر "ابدأ الفحص" لإجراء محاكاة شاملة واختبار المحتوى الحقيقي والمشغل</p>
                      </div>
                      <button 
                        type="button"
                        dir="ltr"
                        onClick={() => {
                          const nextVal = !qaInspectionEnabled;
                          setQaInspectionEnabled?.(nextVal);
                          try {
                            localStorage.setItem('app_qa_inspection_enabled', JSON.stringify(nextVal));
                          } catch {}
                        }}
                        className={`w-14 h-8 rounded-full p-1 transition-colors flex items-center shrink-0 cursor-pointer ${
                          qaInspectionEnabled ? 'bg-emerald-500 justify-end' : 'bg-white/20 justify-start'
                        }`}
                      >
                        <div className="w-6 h-6 bg-white rounded-full shadow-md transition-all" />
                      </button>
                    ...
                    {qaInspectionEnabled ? (
                      <div className="bg-zinc-900 border border-emerald-500/30 p-6 rounded-2xl space-y-4 text-right">
                        <div className="flex items-start gap-3">
                          <Sparkles className="w-6 h-6 text-emerald-400 shrink-0 mt-1" />
                          <div>
                            <h4 className="text-lg font-bold text-emerald-300">جاهز لإطلاق الفحص الشامل والمحاكاة الحقيقية 🎬</h4>
                            <p className="text-xs text-white/70 mt-1 leading-relaxed">
                              يقوم التطبيق بفحص محتوى مكتبتك الحقيقية وقوائم التشغيل والقنوات والجداول والمشغل والتنقل، واكتشاف أية أخطاء أو ملفات مفقودة أو مسارات تالفة مع تقديم تقرير مفصل وإصلاح تلقائي بنقرة واحدة.
                            </p>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full py-4 bg-emerald-500 hover:bg-emerald-400 text-black font-black rounded-xl text-base transition-all shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2 cursor-pointer"
                        >
                          <Play className="w-5 h-5 fill-black" />
                          <span>ابدأ الفحص الشامل الآن (Start QA Inspection)</span>
                        </button>
                      </div>
                    ) : (
                      <div className="p-6 bg-white/5 border border-white/10 rounded-2xl text-center text-white/50 text-xs">
                        قم بتفعيل المفتاح أعلاه لتشغيل وضع الفحص وإجراء اختبار الجودة على مكتبتك الحقيقية.
                      </div>
                    )}
                  </div>
                </div>
      ...
      <ConfirmModal 
        ...
        title="حذف المجلد المحلي"
        message={`هل أنت تأكد من رغبتك في إزالة المجلد "${deletingFolder?.name || ''}" من الإعدادات؟ لن يتم حذف الملفات الأصلية من القرص الصلب.`}
        confirmText="نعم، إزالة المجلد"
        cancelText="إلغاء"
        onConfirm={() => {
          if (deletingFolder) {
